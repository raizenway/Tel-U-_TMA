    ...
    <div className="flex min-h-screen bg-gray-100">
      ...
      <aside className="w-72 bg-white border-r flex flex-col">
        <div className="px-6 py-6">
          <Image src="/Logo.png" alt="Logo Telkom University" width={160} height={60} />
        </div>

        <div className="px-6 py-4 border-y flex items-center space-x-4">
          <Image src="/user-icon.png" alt="User" width={48} height={48} className="rounded-full" />
          <div>
        ...
        <nav className="flex-1 px-4 py-6 space-y-2 text-sm font-medium">
          <button className="w-full flex items-center justify-start space-x-3 text-gray-700 px-3 py-2 rounded hover:bg-gradient-to-r hover:from-red-500 hover:to-gray-500 hover:text-white">
            <span>🏠</span>
            <span>Home</span>
          </button>
          ...
          <button
            ...
            className="w-full flex items-center justify-between text-gray-700 px-3 py-2 rounded hover:bg-gradient-to-r hover:from-red-500 hover:to-gray-500 hover:text-white"
          >
            <div className="flex items-center space-x-3">
              <span>📊</span>
              <span>Assessment Result</span>
            </div>
            {showApproval ? <ChevronUp size={16} /> : <ChevronDown size={16} />}
          </button>
      ...
      <div className="flex-1 flex flex-col">
        {/* Header */}
        <header className="bg-white flex justify-between items-center px-6 py-4 border-b">
        ...
        <div className="flex space-x-6 border-b border-gray-300 px-6 pt-6">
          {[{ name: "Welcome", id: "welcome" }, { name: "Dashboard", id: "dashboard" }, { name: "User Manual", id: "user-manual" }].map((item) => (
            <button
              key={item.id}
        ...
        <main className="p-6 space-y-6">
          ...
          {tab === "dashboard" && (
            <div className="grid grid-cols-1 md:grid-cols-4 gap-9">
              ...
              <div className="bg-amber-500 text-white p-6 shadow rounded">📄 Jumlah Variable & Pertanyaan</div>
              <div className="bg-blue-700 text-white p-6 shadow rounded">📝 Assesment Submitted</div>
              <div className="bg-emerald-600 text-white p-6 shadow rounded">Assesment Approved</div>
            </div>
          )}
