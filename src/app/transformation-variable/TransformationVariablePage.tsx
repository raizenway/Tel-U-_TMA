...
const { data, loading, error, refetch } = useTransformationVariableList();
  ...
  const tableData = Array.isArray(data)
    ? data.map((item, index) => {
        ...
        if (!item || typeof item !== 'object') {
          console.warn(`[ERROR] Item ke-${index} bukan objek:`, item);
          return null;
        }
        ...
        // ✅ Return object dengan semua field
        return {
          id,
          nama: name,
          bobot: weight,
  ...
  const currentData = filteredData
    ...
    const handleToggleStatus = async () => {
      ...
      // ✅ Pastikan data siap dan itemId valid
if (!itemId || !data || !Array.isArray(data)) {
  console.warn("Data belum siap atau itemId tidak valid");
  return;
}
